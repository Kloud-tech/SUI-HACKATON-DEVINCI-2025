...
ddef compute_decision(chimera_state: Dict[str, Any]) -> Dict[str, Any]:
    """
    Calcule la décision génétique sur 3 traits (STR, AGI, INT) :
    lequel manque le plus, et quoi acheter.
    """
    dna: List[int] = chimera_state["dna"]

    try:
        current_str = int(dna[DNA_INDEX_STR])
        current_agi = int(dna[DNA_INDEX_AGI])
        current_int = int(dna[DNA_INDEX_INT])
    except (IndexError, ValueError, TypeError) as e:
        raise ChimeraAgentError(f"Impossible de lire les traits dans dna={dna}: {e}") from e

    missing = {
        "STR": max(TARGET_STR - current_str, 0),
        "AGI": max(TARGET_AGI - current_agi, 0),
        "INT": max(TARGET_INT - current_int, 0),
    }

    # Choisir le trait le plus en déficit
    selected_trait = max(missing, key=missing.get)
    missing_amount = missing[selected_trait]

    if missing_amount > 0:
        action = f"BUY_{selected_trait}"
        reason = f"{selected_trait} en déficit de {missing_amount}"
    else:
        action = "NO_ACTION"
        reason = "Tous les traits sont au-dessus ou au niveau de leur cible"
    ...
    decision = {
        ...
        "missing": missing,                 # manque par trait
        "selected_trait": selected_trait,   # ex: "STR"
        "selected_missing_amount": missing_amount,  # ex: 40
        "action": action,                  # ex: "BUY_STR"
        "reason": reason,
    }
