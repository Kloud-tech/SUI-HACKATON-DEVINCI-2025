...
import { createSuiTools } from "../src/langchain";
...
function validateEnvironment(): void {
  ...
  const requiredVars = ["OPENAI_API_KEY", "RPC_URL", "SUI_PRIVATE_KEY"];
...
async function initializeAgent() {
  try {
    ...
    const agent = createReactAgent({
      ...
      messageModifier: `
        You are a helpful agent that can interact onchain using the Sui Kit. You are
        empowered to interact onchain using your tools. If you ever need funds, you can request them from the
        faucet. If not, you can provide your wallet details and request funds from the user. If there is a 5XX
        (internal) HTTP error code, ask the user to try again later. If someone asks you to do something you
        can't do with your currently available tools, you must say so, and encourage them to implement it
        themselves using the Sui Agent Kit. Be concise and helpful with your responses. 
